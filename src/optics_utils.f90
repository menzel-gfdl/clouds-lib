...
type, public :: OpticalProperties
  real(kind=c_double), dimension(:), allocatable :: bands !< Band center wavenumbers [cm-1] (band).
  real(kind=c_double), dimension(:,:), allocatable :: band_limits !< Band edge wavenumbers [cm-1] (2, band).
  real(kind=c_double), dimension(:), allocatable :: extinction_coefficient !< Extinction coefficient (band).
  real(kind=c_double), dimension(:), allocatable :: single_scatter_albedo !< Single-scatter albedo (band).
  real(kind=c_double), dimension(:), allocatable :: asymmetry_factor !< Asymmetry factor (band).
  contains
...
pure subroutine construct(self, bands, band_limits)
  ...
  real(kind=c_double), dimension(:), intent(in) :: bands !< Band center wavenumbers [cm-1]
  real(kind=c_double), dimension(2,size(bands)), intent(in) :: band_limits !< Band edge wavenumbers [cm-1].
...
pure function interp(x, y, newx) result(newy)
  ...
  real(kind=c_double), dimension(2), intent(in) :: x !< Domain values.
  real(kind=c_double), dimension(2), intent(in) :: y !< Function values
  real(kind=c_double), intent(in) :: newx !< New domain value.
  real(kind=c_double) :: newy !< Interpolated function value.
