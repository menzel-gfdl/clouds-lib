...
type, public :: IncompleteBeta
  ...
  real(kind=c_double), dimension(:), allocatable :: x !< Table input values.
  real(kind=c_double), dimension(:,:,:), allocatable :: y !< Table of calculated values.
  real(kind=c_double), dimension(:,:,:), allocatable :: y_inverse !< Table of inverse values.
  contains
...
pure function interp(x, y, newx) &
  ...
  real(kind=c_double), dimension(:), intent(in) :: x !< Domain values for the array to be interpolated in.
  real(kind=c_double), dimension(:), intent(in) :: y !< Array values to be interpolated in.
  real(kind=c_double), intent(in) :: newx !< Domain value to interpolate to.
  real(kind=c_double) :: newy !< Interpolated value.
...
elemental pure function inverse(self, p, q, x) &
  ...
  real(kind=c_double), intent(in) :: x !< Value to evaluate function at.
  real(kind=c_double) :: y !< Result.
