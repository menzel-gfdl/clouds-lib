...
type, public :: Slingo
  real(kind=c_double), dimension(:), allocatable :: a !< Optical depth coefficients [g-1 m2] (band).
  real(kind=c_double), dimension(:), allocatable :: b !< Optical depth coefficients [g-1 m2 micron] (band).
  real(kind=c_double), dimension(:,:), allocatable :: band_limits !< Parameterization band limits [cm-1] (2, band).
  real(kind=c_double), dimension(:), allocatable :: bands !< Parameterization band centers [cm-1].
  real(kind=c_double), dimension(:), allocatable :: c !< Single-scatter albedo coefficients (band).
  real(kind=c_double), dimension(:), allocatable :: d !< Single-scatter albedo coefficients [micron-1] (band).
  real(kind=c_double), dimension(:), allocatable :: e !< Asymmetry factor coefficients (band).
  real(kind=c_double), dimension(:), allocatable :: f !< Asymmetry factor coefficients [micron-1] (band).
...
elemental pure subroutine optics_(self, water_concentration, equivalent_radius, band, &
  ...
  real(kind=c_double), intent(out) :: extinction_coefficient !< Extinction coefficient.
