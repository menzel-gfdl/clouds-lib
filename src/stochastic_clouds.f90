...
subroutine cloudiness(overlap_parameter, x)
  ...
  real(kind=c_double), dimension(:), intent(in) :: overlap_parameter !< Overlap parameter between adjacent layers.
  real(kind=c_double), dimension(:), intent(out) :: x !< A random number for each layer.

  integer :: i
  real(kind=c_double), dimension(size(x)-1) :: r
...
pure subroutine overlap_parameter(altitude, scale_length, alpha)
  ...
  real(kind=c_double), dimension(:), intent(in) :: altitude !< Layer altitude.
  real(kind=c_double), intent(in) :: scale_length !< Scale length.
  real(kind=c_double), dimension(:), intent(out) :: alpha !< Overlap parameter between adjacent layers.
...
subroutine sample_condensate(self, cloud_fraction, lwc, iwc, overlap, ql, qi)
  ...
  real(kind=c_double), dimension(:), intent(in) :: cloud_fraction !< Saturated volume fraction.
  real(kind=c_double), dimension(:), intent(in) :: lwc !< Cloud liquid water condensate mixing ratio in dry air [kg/kg].
  real(kind=c_double), dimension(:), intent(in) :: iwc !< Cloud ice water condensate mixing ratio in dry air [kg/kg].
  real(kind=c_double), dimension(:), intent(in) :: overlap !< Overlap parameter between adjacent layers.
  real(kind=c_double), dimension(:), intent(out) :: ql !< Cloud liquid water condensate mixing ratio in dry air [kg/kg].
  real(kind=c_double), dimension(:), intent(out) :: qi !< Cloud ice water condensate mixing ratio in dry air [kg/kg].

  real(kind=c_double), dimension(size(cloud_fraction)) :: liquid_fraction
  real(kind=c_double), dimension(size(cloud_fraction)) :: qs
  real(kind=c_double), dimension(size(cloud_fraction)) :: total_condensate
  real(kind=c_double), dimension(size(cloud_fraction)) :: width
  real(kind=c_double), dimension(size(cloud_fraction)) :: x
...
elemental pure function specific_saturation_humidity(self, cloud_fraction) &
  ...
  class(TotalWaterPDF), intent(in) :: self
  real(kind=c_double), intent(in) :: cloud_fraction !< saturated volume fraction.
  real(kind=c_double) :: qs !< Normalized saturation specific humidity in dry air [kg/kg].
...
elemental pure function width(self, cloud_fraction, lwc, iwc, qs) &
  ...
  class(TotalWaterPDF), intent(in) :: self
  real(kind=c_double), intent(in) :: cloud_fraction !< Saturated volume fraction.
  real(kind=c_double), intent(in) :: lwc !< Cloud liquid water condensate mixing ratio in dry air [kg/kg].
  real(kind=c_double), intent(in) :: iwc !< Cloud ice water condensate mixing ratio in dry air [kg/kg].
  real(kind=c_double), intent(in) :: qs !< Normalized saturation specific humidity in dry air [kg/kg].
  real(kind=c_double) :: w !< Distribution width (b - a).

  w = (lwc + iwc)/((real(self%p, kind=c_double)/real(self%p + self%q, kind=c_double))* &
      (1._c_double - self%beta%value(self%p + 1, self%q, qs)) - qs*cloud_fraction)
end function width
