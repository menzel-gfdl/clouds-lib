...
type, public :: HuStamnes
  real(kind=c_double), dimension(:,:), allocatable :: a1 !< Exctinction coefficient parameter (band, radius).
  real(kind=c_double), dimension(:,:), allocatable :: a2 !< Single-scatter albedo parameter (band, radius).
  real(kind=c_double), dimension(:,:), allocatable :: a3 !< Asymmetry factor parameter (band, radius).
  real(kind=c_double), dimension(:,:), allocatable :: band_limits !< Lower/upper bounds of parameterization [cm-1] (2, band).
  real(kind=c_double), dimension(:), allocatable :: bands !< Parameterization band centers [cm-1] (band).
  real(kind=c_double), dimension(:,:), allocatable :: b1 !< Exctinction coefficient parameter (band, radius).
  real(kind=c_double), dimension(:,:), allocatable :: b2 !< Single-scatter albedo parameter (band, radius).
  real(kind=c_double), dimension(:,:), allocatable :: b3 !< Asymmetry factor parameter (band, radius).
  real(kind=c_double), dimension(:,:), allocatable :: c1 !< Exctinction coefficient parameter (band, radius).
  real(kind=c_double), dimension(:,:), allocatable :: c2 !< Single-scatter albedo parameter (band, radius).
  real(kind=c_double), dimension(:,:), allocatable :: c3 !< Asymmetry factor parameter (band, radius).
  integer :: last_ir_band !< Index of last infrared (longwave) band.
  real(kind=c_double) :: max_radius !< Maximum radius defined in parameterization [micron].
  real(kind=c_double) :: min_radius !< Minimum radius defined in parameterization [micron].
  real(kind=c_double), dimension(:), allocatable :: radii !< Radius bins [micron] for parameterization (radius).
  contains
...
subroutine construct(self, path)
  ...
  integer :: ncid
  real(kind=c_double), dimension(2) :: radii
  real(kind=c_double), dimension(:,:), allocatable :: radius_bounds
...
elemental pure subroutine optics(self, water_concentration, equivalent_radius, &
  ...
  real(kind=c_double), intent(in) :: water_concentration !< Water concentration [g m-3].
...
elemental pure subroutine optics_(self, water_concentration, equivalent_radius, band, &
  ...
  class(HuStamnes), intent(in) :: self
  real(kind=c_double), intent(in) :: water_concentration !< Water concentration [g m-3].
  real(kind=c_double), intent(in) :: equivalent_radius !< Droplet equivalent radius [micron].
  ...
  real(kind=c_double), parameter :: m_to_km = 1.e-3_c_double
  integer :: i
  ...
  single_scatter_albedo = 1._c_double - (self%a2(band,i)*(r**self%b2(band,i)) + self%c2(band,i)) !Equation 14.
